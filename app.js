...
const STORAGE_KEYS = {
  ...
  defaults: 'dt_v1_defaults', // targets
...
// Defaults
const DEFAULT_TARGETS = { calories: 1820, protein: 163, carbs: 165, fat: 41 };

// A starting library inspired by the screenshot provided
...
function renderItemsTable() {
  ...
  for (const item of state.day.items) {
    ...
    const consumed = item.consumedServings * item.calories;
    consumedCell.textContent = round0(consumed);
...
function renderTotals() {
  ...
  qs('#totalConsumed').textContent = round0(totals.calories);
...
function attachEvents() {
  ...
  qs('#itemsTbody').addEventListener('input', (e) => {
    ...
    if (e.target.classList.contains('planned')) {
      ...
      // keep consumed within plan if checkbox is still checked
      if (item.consumedServings > item.plannedServings) {
        item.consumedServings = item.plannedServings;
      }
      saveDay(state.day);
  ...
  form.addEventListener('close', () => {
    ...
    if (data.saveToLibrary === 'on') {
      // add to library if not present by name
  ...
  // Import/Export (guarded, may be absent if Tools section hidden)
  const exportBtn = qs('#exportBtn');
  if (exportBtn) {
    exportBtn.addEventListener('click', () => {
      const blob = new Blob([
        JSON.stringify({
          meta: { app: 'diet-tracker', version: 1 },
          targets: state.targets,
          library: state.library,
          template: loadJson(STORAGE_KEYS.template, null),
          days: Object.keys(localStorage)
            .filter((k) => k.startsWith('dt_v1_day_'))
            .map((k) => loadJson(k, null))
            .filter(Boolean),
        }, null, 2)
      ...
      a.href = url;
      a.download = `diet-tracker-export-${state.dateStr}.json`;
      a.click();
  ...
  if (importInput) {
    importInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        if (data.targets) saveJson(STORAGE_KEYS.defaults, data.targets);
        if (data.library) saveJson(STORAGE_KEYS.library, data.library);
        if (data.template) saveJson(STORAGE_KEYS.template, data.template);
        if (Array.isArray(data.days)) {
          for (const day of data.days) {
            if (day && day.date) saveJson(STORAGE_KEYS.day(day.date), day);
          }
        }
        // reload after import
        loadAllState();
